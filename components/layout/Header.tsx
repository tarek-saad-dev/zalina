  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out ${
        isScrolled
          ? "bg-[#0B0B0F]/95 backdrop-blur-md border-b border-[#D4A95A]/10"
          : "bg-transparent"
      }`}
    >
      <div className="w-full px-6 sm:px-8 lg:px-12 xl:px-16">
        <nav className="flex items-center justify-between h-20 lg:h-24">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <span className="hidden sm:block text-[#F5E9DA] font-serif text-lg lg:text-xl tracking-wide">
              Zalina
            </span>
          </Link>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`relative text-sm tracking-[0.15em] uppercase transition-colors duration-300 ${
                  item.active
                    ? "text-[#D4A95A]"
                    : "text-[#F5E9DA]/80 hover:text-[#F5E9DA]"
                }`}
              >
                {item.label}
                {item.active && (
                  <span className="absolute -bottom-1 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#D4A95A] to-transparent" />
                )}
              </Link>
            ))}
          </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden absolute top-full left-0 right-0 bg-[#0B0B0F]/98 backdrop-blur-lg border-b border-[#D4A95A]/10 transition-all duration-500 ease-out ${
          isMobileMenuOpen
        ...
        <div className="px-6 py-8 space-y-6">
          {navItems.map((item, index) => (
            <Link
              ...
              className={`block text-sm tracking-[0.15em] uppercase transition-all duration-300 ${
                item.active
                  ? "text-[#D4A95A]"
